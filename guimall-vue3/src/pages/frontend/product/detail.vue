<template>
  <div class="min-h-screen bg-stone-50 font-sans pb-20">
      ...
      <div class="grid grid-cols-1 lg:grid-cols-12 gap-12">
        ...
        <div class="lg:col-span-6 space-y-8">
          ...
          <div class="pt-8 flex flex-col sm:flex-row gap-4">
            ...
            <button @click="handleBuyNow" class="flex-1 bg-emerald-600 text-white py-4 rounded-2xl font-black text-lg shadow-lg shadow-emerald-200 hover:bg-emerald-700 hover:-translate-y-1 transition-all">立即购买</button>
          </div>
    ...
    <div class="lg:hidden fixed bottom-0 left-0 right-0 bg-white/80 backdrop-blur-lg border-t border-stone-200 p-4 flex gap-4 z-50">
      ...
      <button @click="handleBuyNow" class="flex-1 bg-emerald-600 text-white font-black rounded-2xl">立即购买</button>
    </div>
  </div>
</template>
...
// 立即购买：校验登录和规格，构造订单商品数据，存入 sessionStorage 后跳转结算页
const handleBuyNow = () => {
  if (!isMemberLoggedIn()) {
    message.warning('请先登录')
    router.push(`/member/login?redirect=${encodeURIComponent(route.fullPath)}`)
  ...
  // 构造订单商品数据（不包含 cartItemId，因为不是从购物车来的）
  const orderItem = {
    productId: product.value.id,
    productSkuId: selectedSku.value.id,
    productName: product.value.name,
